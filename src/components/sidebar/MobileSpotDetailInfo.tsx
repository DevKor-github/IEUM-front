...
const SpotDetailInfo = (props: SpotDetailInfoPropsType) => {
  ...
  const [instaContent, setInstaContent] = useState<string>(code);
  ...
  const getSpotInfo = async () => {
    let spot = spotListState?.find(
      (item) => item.spotContent.instaGuestCollectionId == selectedId,
    )?.spotContent;

    if (!spot && !!selectedId) {
      spot = await getInstaCollectionDetail(params?.userId || '', selectedId);
    }
  ...
  return (
    <>
      <div className="imbed" dangerouslySetInnerHTML={{ __html: code }}></div>
